  ...
  public static void TriggerPlantBatch()
  {
    if (autosaveNum == 0)
    {
      foreach (Map map in Find.Maps)
      {
        ...
        if (spawner != null)
        {
          Thing plantSpawner = ThingMaker.MakeThing(MOL_DefOf.MOL_PlantSpawner);
          GenSpawn.Spawn(plantSpawner, IntVec3.Zero, map);
        }
        System.Threading.Thread.Sleep(ticksToSpreadOver * 17);
      }
